...
"""
Hyperparameter Search for 14-Class Guitar Chord Classification
This version handles all 14 chord classes including "E" (E major).
"""
...
def load_dataset(dataset_path, test_size=0.2, val_size=0.125):
    ...
    Automatically detects all chord folders (should be 14 classes including "E").
    """
...
def run_coarse_search(dataset_path):
    ...
    print(f"  Number of classes: {num_classes} (expected 14)")
    print(f"  Train: {len(train_paths)} images")
    print(f"  Val: {len(val_paths)} images")
    print(f"  Test: {len(test_paths)} images (held out)")
    
    if num_classes != 14:
        print(f"\n⚠ Warning: Expected 14 classes but found {num_classes}")
    ...
    # Save results
    with open('hyperparameter_search_3_coarse.json', 'w') as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to hyperparameter_search_3_coarse.json")
...
def run_fine_search(coarse_results, train_data, val_data, num_classes):
    ...
    print(f"  Batch Size: {best_coarse['batch_size']}")
    print(f"  Dropout: {best_coarse['dropout']}")
    
    # Determine fine-tuning grid based on coarse results
    # Learning rate fine-tuning
    lr = best_coarse['lr']
    if lr == 0.01:
        lr_fine = [0.01, 0.005, 0.001]
    elif lr == 0.001:
        lr_fine = [0.001, 0.0005, 0.0001]
    elif lr == 0.0001:
        lr_fine = [0.0001, 0.00005, 0.00001]
    elif lr == 0.00001:
        lr_fine = [0.00001, 0.000005, 0.000001]
    else:
        lr_fine = [lr]
    
    # Epochs fine-tuning
    epochs = best_coarse['epochs']
    if epochs == 10:
        epochs_fine = [10, 15, 20]
    elif epochs == 20:
        epochs_fine = [15, 20, 25]
    elif epochs == 30:
        epochs_fine = [25, 30, 35]
    else:
        epochs_fine = [epochs]
    
    # Batch size fine-tuning (around best value)
    ...
    if batch_size == 16:
        ...
        batch_fine = [batch_size]
    
    # Dropout fine-tuning (around best value)
    dropout = best_coarse['dropout']
    if dropout == 0.1:
        dropout_fine = [0.05, 0.1, 0.15]
    elif dropout == 0.2:
        dropout_fine = [0.15, 0.2, 0.25]
    elif dropout == 0.3:
        dropout_fine = [0.25, 0.3, 0.35]
    elif dropout == 0.4:
        dropout_fine = [0.35, 0.4, 0.45]
    else:
        dropout_fine = [dropout]
    
    hp_grid = {
        'lr': lr_fine,
        'epochs': epochs_fine,
        'batch_size': batch_fine,
        'dropout': dropout_fine
    }
    ...
    # Save results
    with open('hyperparameter_search_3_fine.json', 'w') as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to hyperparameter_search_3_fine.json")
    
    return results

def plot_training_history(history, save_path='training_history_3.png'):
...
def train_final_model(best_config, train_data, val_data, test_data, class_to_idx, idx_to_class):
    ...
    print("\n" + "="*80)
    print("TRAINING FINAL MODEL WITH BEST HYPERPARAMETERS (14 CLASSES)")
    print("="*80)
    ...
    # Save class mapping
    with open('class_mapping_3.json', 'w') as f:
        json.dump({'class_to_idx': class_to_idx, 'idx_to_class': idx_to_class}, f, indent=2)
    print("\n✓ Class mapping saved to class_mapping_3.json")
    ...
    checkpoint = torch.load('best_chord_model_3.pth')
    ...
    print(f"\nFiles saved:")
    print(f"  • best_chord_model_3.pth - Trained model (14 classes)")
    print(f"  • class_mapping_3.json - Class to index mapping (14 classes)")
    print(f"  • training_history_3.png - Training curves")
    print(f"{'='*80}")
...
def main():
    ...
    print("\n" + "="*80)
    print("GUITAR CHORD CLASSIFICATION - HYPERPARAMETER SEARCH (14 CLASSES)")
    print("="*80)
